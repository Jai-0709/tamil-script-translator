...
======================
Generates a comprehensive, publication-grade Master Documentation PDF for
the Classical Tamil Epigraphy Suite v2.0.

Includes:
1. Executive Summary & Historical Epigraphy Background
2. Complete Layered Technology Stack
3. End-to-End Computational Architecture & Detailed Dataflow
4. Development Journey from Scratch & Solved Engineering Challenges
5. Full Production Source Code (Formatted & Paginated):
   - Gemini Epigraphic Engine (backend/gemini_engine.py)
   - Universal Segmentation & Option 4 Per-Line Engine (backend/segmentation.py)
   - 247-Class Classifier Engine (backend/classifier.py)
   - Kaggle YOLOv8 Training Script (scripts/training/kaggle_yolo_train.py)
   - Kaggle 247-Class Classifier Training Script (scripts/training/kaggle_train.py)
...
# ── Colors & Palettes ────────────────────────────────────────────────────────
C_PRIMARY     = colors.HexColor("#1e293b")   # Slate Navy
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Header
        self.drawString(54, 11 * inch - 36, "Classical Tamil Epigraphy Suite (v2.0) — Technical Architecture & Master Documentation")
        ...
        self.drawString(54, 32, "Confidential & Academic Department Project — Autonomous OCR & AI Translation Engine")
        page_str = f"Page {self._pageNumber} of {page_count}"
        self.drawRightString(8.5 * inch - 54, 32, page_str)
        self.restoreState()


def get_code_chunks(file_path: Path, max_total_lines: int = 350, chunk_size: int = 50):
...
def build_pdf(filename: str):
    ...
    story.append(Paragraph("🏛️ CLASSICAL TAMIL EPIGRAPHY SUITE (v2.0)", style_cover_title))
    story.append(Paragraph(
        "Master Technical Report: Computational Epigraphy Architecture, Deep Learning (YOLOv8 + ViT), "
        "LLM Context Refinement (Gemini 2.5 Flash), and Complete Production Source Code",
    ...
    meta_table_data = [
        [Paragraph("<b>Project Version:</b>", style_body), Paragraph("Version 2.0 (High-Precision Epigraphy Suite)", style_body)],
        [Paragraph("<b>Domain Areas:</b>", style_body), Paragraph("Computational Epigraphy, Deep Vision, Document OCR, Multimodal LLMs", style_body)],
        ...
        [Paragraph("<b>Deep Vision Models:</b>", style_body), Paragraph("Ultralytics YOLOv8m (Character Region Segmentation) & 247-Class Classifier", style_body)],
        [Paragraph("<b>LLM Epigraphic Engine:</b>", style_body), Paragraph("Google Gemini 2.5 Flash API (Epigraphic Word Spacing & Metric Conservation)", style_body)],
    ...
    exec_summary_text = (
        "<b>Executive Overview:</b> Classical South Indian stone inscriptions (கல்வெட்டுகள்), palm-leaf "
        "manuscripts, and temple rubbings contain invaluable historical data from the Chola, Pandya, and Pallava "
        "eras. Due to surface erosion, irregular stone textures, and compound ligatures (Grantha / Vatteluttu), "
        "traditional OCR systems fail. The Classical Tamil Epigraphy Suite v2.0 solves these fundamental hurdles "
        "through an end-to-end autonomous architecture combining: (1) Sliced YOLOv8 character segmentation with "
        "Option 4 Per-Line Assembly, (2) a 247-class deep character classifier, (3) real-time few-shot Vector Memory, "
        "and (4) Gemini 2.5 Flash epigraphic context refinement that preserves ancient gold weight metrics (கழஞ்சு, மஞ்சாடி) "
        "and delivers structured line-by-line Modern Tamil and fluent English translations."
    )
    ...
    story.append(Paragraph("<b>Paleographical & Epigraphic Challenges:</b>", style_h3))
    story.append(Paragraph("• <b>Stone Texture & Weathering:</b> Pitting, cracks, and uneven groove illumination cause false positives. Resolved using adaptive Gaussian-Otsu binarization and morphological filtering.", style_bullet))
    story.append(Paragraph("• <b>Compound Glyphs:</b> Ancient Tamil combines consonants with vowel modifiers into indivisible compound glyphs (e.g. பொ, கொ, ஶ்ரீ). Traditional OCR algorithms segment them into partial strokes; our YOLOv8 model preserves whole glyph boxes.", style_bullet))
    story.append(Paragraph("• <b>Scriptio Continua:</b> Inscriptions lack word spaces. The Gemini 2.5 engine separates words contextually while retaining ancient measurement units.", style_bullet))
    ...
    tech_table_data = [
        ...
        [Paragraph("Frontend UI", style_body), Paragraph("React 18 + Vite 5", style_body), Paragraph("High-speed SPA with fast hot-module reload and dynamic routing.", style_body)],
        ...
        [Paragraph("Segmentation AI", style_body), Paragraph("Ultralytics YOLOv8m", style_body), Paragraph("Trained character segmenter with tiled inference (`models/best.pt`).", style_body)],
    ...
    pipeline_steps = [
        ...
        ("Step 6: Gemini 2.5 Flash Epigraphic Analysis & Translation",
         "Detected characters and line groups are sent to Gemini 2.5 Flash with specialized epigraphic system instructions. Gemini restores word spacing, translates to Modern Tamil & English, and preserves ancient Chola gold metrics (கழஞ்சு, மஞ்சாடி)."),
    ...
    for title, text in challenges:
        ...
        box = Table([[Paragraph(text, style_body)]], colWidths=[6.8 * inch])
    ...
    quickstart_text = (
        "<b>1. Local Execution:</b><br/>"
        "• Clone: <code>git clone https://github.com/Jai-0709/tamil-script-translator.git</code><br/>"
        "• Backend: <code>python -m venv venv && venv\\Scripts\\activate && pip install -r backend/requirements.txt</code><br/>"
        "• Gemini Key: <code>set GEMINI_API_KEY=your_actual_key</code><br/>"
        "• Launch Backend: <code>python app.py</code> (runs on port 8000)<br/>"
        "• Frontend: <code>cd frontend && npm install && npm run dev</code> (opens on port 5173)<br/><br/>"
        "<b>2. Cloud Deployment:</b><br/>"
        "• <b>Frontend:</b> Deploy to Vercel / Netlify with environment variable <code>VITE_BACKEND_URL</code>.<br/>"
        "• <b>Backend:</b> Deploy to Hugging Face Spaces (Docker / Python SDK) or Render.<br/>"
        "• <b>Bypass Headers:</b> Built-in tunnel bypass headers ensure seamless execution over LocalTunnel or ngrok without authentication popups."
    )
...
if __name__ == "__main__":
    out_pdf = "Classical_Tamil_Epigraphy_Suite_Master_Documentation.pdf"
    build_pdf(out_pdf)
